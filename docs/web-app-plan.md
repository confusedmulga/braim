...
> **Build status (2026-09-25).** Phases 0–5 are built on this branch, plus the
> Phase 6 font work and right-click. Verified in the cloud container: `flutter
> test` (218 tests, including the phone server over real sockets), `flutter
> analyze`, `flutter build web` and `flutter build apk --debug`, and two
> Playwright runs in Chromium: `tool/web_smoke.mjs` (browser-only library) and
> `tool/remote_smoke.mjs` (the real phone server over a real SQLite library,
> serving the real web build). **Not verified: anything on a physical phone**
> (install over an existing library, a real hotspot, biometrics, screen-off
> behaviour) — there is no device in the container. Deviations from this plan
> are marked *Built as* below.
...
- [x] Install Flutter (stable with Dart ≥ 3.11.5, per `pubspec.yaml`) in the
      container; `flutter doctor`; `flutter test` green on untouched code.
      *Built as:* Flutter 3.41.8 (the project's own revision). Gradle needs
      Maven Central through a mirror in this container (an init script in
      `~/.gradle/init.d`, not in the repo) because the proxy rate-limits it.
- [x] `flutter create --platforms web .`; commit `web/`.
- [x] Make `flutter build web` compile: introduce `lib/platform/` stubs and
      conditional imports until there are zero `dart:io` imports outside
      allowed files. No behaviour change on Android. *Built as:* `dart:io` is
      left in the phone-only services (storage, backup, DND, focus media);
      everything a browser runs goes through `lib/platform/`, and
      `package:web` sits behind one conditional import (`web_bridge.dart`).
- [x] Android: `flutter test`, `flutter build apk --debug` green.

### Phase 1 — Seams (Android behaviour unchanged)
- [x] Extract `SnapshotDiffer` from `DbStore`; `db_store_test.dart` green unchanged; new differ tests.
- [x] `LibraryStore` + `DeviceLibraryStore`; `AppState(store:)`; move init/flush/flushNow store logic into it.
- [x] `AppState.applyIncoming(delta)` + tests (insert, replace, hard delete, settings, no echo).
- [x] `BraimImage` / `ImageStore` / `FileSaver` / `FileOpen` / caps; replace all 13 `Image.file`/`FileImage`, all export/share call sites, `_importFile`, crop.
- [x] Relative image paths resolve on Android.
- [x] `Fetcher` injection for link previews / YouTube.
- [x] Settings scope split (§7), no-op on Android. *Built as:* the split lives
      in `settings_scope.dart` and is applied by the remote store and the phone
      server (the only place both sides write one library).
- [ ] Regression: install debug APK on device over an existing library; everything opens, edits persist across restart, backup/restore still works. **Needs a phone.**

### Phase 2 — Local mode MVP (browser only, no phone needed)
- [x] `WebLocalLibraryStore`; images in IndexedDB. *Built as:* rows in
      IndexedDB (same `"<table>:<id>" → row` shape and the same differ) instead
      of sqflite-on-WASM: no 700 KB engine or worker to ship, no GitHub download
      at build time. Search stays in memory (the FTS fallback that exists).
- [x] Empty-library screen with "Import backup from phone"; import zip (§11.2).
- [x] All screens render the imported library (screenshots in light and dark).
- [x] Create/edit/delete notes; reload the tab; changes persist (smoke test).
- [x] Export zip in the phone's restore format (unit-tested round trip). Restore on a phone: **needs a phone.**
- [x] Caps hide: device backups, notifications, camera, share intent, DND; link previews deferred.

### Phase 3 — Desktop frame and input
- [x] `DesktopFrame` (§8) with wallpaper backdrop; light/dark follows `prefers-color-scheme` when "follow system" is on.
- [x] Right-click = long-press everywhere a long-press exists; browser context
      menu disabled. *Built as:* one handler in the web frame hit-tests the
      click and runs the innermost long-press (as accessibility does), so no
      widget was edited; text fields keep right-click.
- [x] Mouse drag on main PageView; wheel/trackpad scroll everywhere.
- [x] Keyboard shortcuts; browser Back pops routes. Also fixed: focus parks at
      the root when a text field closes (keys stopped reaching the app), and a
      crash in the framework's focus restore on view focus.
- [ ] Editor: link insert/edit menu, `[[` picker, `@@@` mentions with keyboard + mouse — typing, selection and Ctrl+Enter are covered by the smoke test; the pickers are not yet exercised.

### Phase 4 — Phone server, read-only remote mode
- [x] `PhoneServer` + Settings "Open on computer" sheet with QR + URL.
- [x] Bundle build script (`tool/build_web_bundle.sh`); bundle served from the
      APK's native assets via `braim/webbundle`; `/assets/assets/*` from
      `rootBundle`. APK growth ≈ 11 MB compressed (main.dart.js 8.8 MB,
      CanvasKit 13 MB, core fallback fonts 3.6 MB, before compression).
- [x] `/api/hello`, pairing + confirm dialog + token, snapshot, `/api/image`.
      *Built as:* the snapshot is the first frame on `/api/live` (atomic with
      later pushes); `GET /api/snapshot` exists for tools. Requests must be
      addressed to an IP (DNS-rebinding guard); the socket checks `Origin`.
- [x] Web detects remote mode, pairs, renders the phone's library. Crypt withheld.
- [ ] Hotspot / home Wi-Fi with mobile data off. **Needs a phone.** (The smoke
      test fails on any request leaving the machine.)

### Phase 5 — Remote writes and live updates
- [x] `/api/live` both directions; `RemoteLibraryStore.push` sends differ output; acks.
- [x] Phone edits appear in the browser within ~1 s and vice versa.
- [x] Side effects run on the phone for browser edits (reminders re-armed; FTS via the DB write).
- [x] Open-editor rule (§10.6) on both sides (note editor: reading swaps in the new version; writing offers "Load").
- [x] Image upload before row push (`PUT` returns before the path is used).
- [x] `/api/fetch` proxy for link previews (authenticated, http(s), 3 MB, 10 s).
- [x] `/api/search` used for search in remote mode.
- [x] Crypt unlock via phone biometric; auto re-lock after 10 idle minutes (activity pings).
- [x] Disconnect/reconnect (§10.4). *Built as:* after 60 s out of reach the
      page is covered by a blocking notice rather than a partial read-only
      mode (edits can't be intercepted cheaply after they happen in memory).
- [ ] Foreground service. *Built as:* documented limitation; the screen is kept
      on while the server runs.

### Phase 6 — Polish
- [ ] Hover cursors, tooltips on icon buttons.
- [ ] Drag-and-drop images/links/markdown files onto the window.
- [ ] PWA install for local mode (the manifest and icons are ready; build with
      `--pwa-strategy=offline-first` for a secure-context host).
- [x] Paired-devices list with revoke; server auto-stop (15 min idle).
- [x] Offline fonts: fallback fonts are served from the app's own origin
      (`tool/fetch_web_fonts.py`, `fontFallbackBaseUrl`); the smoke tests
      assert no request leaves the machine.
- [ ] Try `--wasm` (the dry run passes).
